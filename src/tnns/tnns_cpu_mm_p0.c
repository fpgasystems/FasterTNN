...
static float* tnn_conv_p0(float* x, float* q_thresholds, int64_t* q_weights, int H_pad, int W_pad, int H_stride, int W_stride, int N, int C, int H, int W, int KN, int KH, int KW, float prelu_alpha) {
    ...
    int64_t *qx = t2r_p0(x, q_thresholds, N, C, H, W, H_pad, W_pad, KH, KW, H_stride, W_stride);
...
void tnns_gemm(testcase_tnns_gemm_t* data) {
    float prelu_alpha = 1;
    float* result = mm_tnn(data->x_packing0, data->w_packing0, data->m, 1, 1, data->n, data->p_packed, prelu_alpha);
