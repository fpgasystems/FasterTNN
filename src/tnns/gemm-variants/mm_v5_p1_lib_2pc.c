...
static inline void micro_mm_p1_lib_2pc(int64_t* a, int64_t* b, int* y, int I, int J, int K, int N, int KB, float alpha, bool last) {
    for (int i = 0; i < I; i++) {
        for (int j = 0; j < J; ++j) {
            ...
            for(; k+(BITS*4) <= K; k+=(BITS*4)) {
                __m256i a_sign = _mm256_loadu_si256((__m256i*)(a + (i*KB + k + 0)));
                __m256i a_abs = _mm256_loadu_si256((__m256i*)(a + (i*KB + k + 4)));
                __m256i b_sign = _mm256_loadu_si256((__m256i*)(b + (j*KB + k + 0)));
                __m256i b_abs = _mm256_loadu_si256((__m256i*)(b + (j*KB + k + 4)));

                __m256i p1 = _mm256_xor_si256(a_sign, b_sign);
                __m256i p2 = _mm256_and_si256(a_abs, b_abs);
            ...
            for (; k < K; k += BITS) {
                ...
                int64_t p2 = a[i*KB + k + 4] & b[j*KB + k + 4];
                cntp1_0 += popcnt64(p2);
...
static float* mm_p1_lib_2pc(int64_t* a, int64_t* b, int M, int N, int K, float prelu_alpha) {
    mm_blocked(4, 1, KB, micro_mm_p1_lib_2pc)
}

/**
 * Optimization 5:
 * We switch to a different bitpacking which allows us to get rid of unpacking instructions
*/
float* mm_tnn(int64_t* a, int64_t* b, int N, int OH, int OW, int KN, int K, float prelu_alpha) {
    return mm_p1_lib_2pc(a,b,N*OH*OW,KN,K,prelu_alpha);
}
