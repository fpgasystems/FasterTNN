...
static int32_t p2_lo[8] = {0,0,0,0,0,0,0,0};
static int32_t p1_hi[8] = {0,0,0,0,0,0,0,0};
static int32_t p2_hi[8] = {0,0,0,0,0,0,0,0};


static inline void t2r_ARMx2(float* x, int64_t* qx_row, float* q_thresholds, int64_t* onebit, int C, int H, int W, int in, int ic, int ih, int iw, int ih_pad, int iw_pad, int H_stride, int W_stride, int kH, int kW, int C_pack, int H_fused, int W_fused, int OH, int OW) {
    // initialize vectors
    int64x2_t p1 = vdupq_n_s64(0);
    int64x2_t p2 = vdupq_n_s64(0);

    float32x2_t threshold = vdup_n_f32(q_thresholds[in]);
    float32x2_t neg_threshold = vdup_n_f32(-q_thresholds[in]);
    
    // convert float32x2_t to float64x2_t
    float64x2_t threshold64 = vcvt_f64_f32(threshold);
    float64x2_t neg_threshold64 = vcvt_f64_f32(neg_threshold);
    ...
    for (int64_t bit = 0; bit < cntbits; bit++) {
        ...
        int64x2_t mask1 = vcltq_f64(currentx64, neg_threshold64);
        // compare current values with positive threshold
        int64x2_t mask2 = vcgtq_f64(currentx64, threshold64);
        // bitwise or of mask1 and mask2
        mask2 = vorrq_s64(mask1, mask2);
        ...
        p2 = vorrq_s64(p2, vandq_s64(mask2, bitvec));
    }
    ...
    // combine lower lane of p1 and p2 and upper lane of p1 and p2
    int64x2_t p_iw0 = vcombine_s64(vget_low_s64(p1), vget_low_s64(p2));
    int64x2_t p_iw1 = vcombine_s64(vget_high_s64(p1), vget_high_s64(p2));

    while((bh_base >= 0) && (bw_base < kH * kW)) {
        int bh = min(iw_pad / W_stride, OW-1);
        int bw = max(iw_pad - (OW-1)*W_stride, iw_pad % W_stride);

        int index_low = (in * H_fused + bh_base + bh) * W_fused + ((bw_base + bw) * C_pack + ic) * BITS;
        int64_t iteration_diff = -W_fused + W_stride * C_pack * BITS;

        for (; (bh >= 0) && (bw < kW); bh -= 1, bw += W_stride) {
            // out[n, bh_base+bh, bw_base+bw, ci, 0] = p1
            // out[n, bh_base+bh, bw_base+bw, ci, 1] = p2
            // store p_iw0 to qx_row
            vst1q_s64(&qx_row[index_low], p_iw0);
            index_low += iteration_diff;
        }
        
        bh = min((iw_pad + 1) / W_stride, OW-1);
        bw = max(iw_pad + 1 - (OW-1)*W_stride, (iw_pad + 1) % W_stride);

        int index_high = (in * H_fused + bh_base + bh) * W_fused + ((bw_base + bw) * C_pack + ic) * BITS;

        for (; (bh >= 0) && (bw < kW); bh -= 1, bw += W_stride) {
            vst1q_s64(&qx_row[index_high], p_iw1);
            index_high += iteration_diff;
        }
...
static inline void t2r_ARMx4(float* x, int64_t* qx_row, float* q_thresholds, int C, int H, int W, int in, int ic, int ih, int iw, int ih_pad, int iw_pad, int H_stride, int W_stride, int kH, int kW, int C_pack, int H_fused, int W_fused, int OH, int OW) {

    // initialize vectors
    int32x4_t v_p1 = vdupq_n_s32(0);
    int32x4_t v_p2 = vdupq_n_s32(0);
    int32x4_t v_onebit = vdupq_n_s32(1);

    float32x4_t v_pos_threshold = vdupq_n_f32(q_thresholds[in]);
    float32x4_t v_neg_threshold = vdupq_n_f32(-q_thresholds[in]);
    ...
    for (int bit = 0; bit < limit; bit++) {
        ...
        // compare current values with negative threshold
        uint32x4_t v_p1_mask = vcltq_f32(v_currentx, v_neg_threshold);
        // compare current values with positive threshold
        uint32x4_t v_p2_mask = vcgtq_f32(v_currentx, v_pos_threshold);
        // p2 bit is also one if p1 is one -> p2_mask = p1_mask OR p2_mask
        v_p2_mask = vorrq_s32(v_p1_mask, v_p2_mask);
        ...
        uint32x4_t v_p2_nextbit = vandq_s32(v_p2_mask, v_onebit);

        // insert the bits using OR
        v_p1 = vorrq_s32(v_p1, v_p1_nextbit);
        v_p2 = vorrq_s32(v_p2, v_p2_nextbit);
    ...
    int bh, bw, idx;
    int it_diff = -W_fused + W_stride * C_pack * BITS;
    while((bh_base >= 0) && (bw_base < kH * kW)) {
        for (int i = 0; i < 4; i++) {
            bh = min((iw_pad + i) / W_stride, OW-1);
            bw = max(iw_pad + i - (OW-1)*W_stride, (iw_pad + i) % W_stride);
            idx = (in * H_fused + bh_base + bh) * W_fused + ((bw_base + bw) * C_pack + ic) * BITS;
...
static inline void t2r_ARMx8(float* x, int64_t* qx_row, float* q_thresholds, int C, int H, int W, int in, int ic, int ih, int iw, int ih_pad, int iw_pad, int H_stride, int W_stride, int kH, int kW, int C_pack, int H_fused, int W_fused, int OH, int OW) {

    // initialize vectors
    int32x4_t v_p1_0 = vdupq_n_s32(0);
    int32x4_t v_p1_1 = vdupq_n_s32(0);
    int32x4_t v_p2_0 = vdupq_n_s32(0);
    int32x4_t v_p2_1 = vdupq_n_s32(0);
    int32x4_t v_onebit = vdupq_n_s32(1);
    ...
    for (int bit = 0; bit < limit; bit++) {
        ...
        // compare current values with negative threshold
        uint32x4_t v_p1_mask_0 = vcltq_f32(v_currentx_0, v_neg_threshold);
        uint32x4_t v_p1_mask_1 = vcltq_f32(v_currentx_1, v_neg_threshold);
        // compare current values with positive threshold
        uint32x4_t v_p2_mask_0 = vcgtq_f32(v_currentx_0, v_pos_threshold);
        uint32x4_t v_p2_mask_1 = vcgtq_f32(v_currentx_1, v_pos_threshold);
        // p2 bit is also one if p1 is one -> p2_mask = p1_mask OR p2_mask
        v_p2_mask_0 = vorrq_s32(v_p1_mask_0, v_p2_mask_0);
        v_p2_mask_1 = vorrq_s32(v_p1_mask_1, v_p2_mask_1);

        // set bit to 1 if the mask is one -> AND the mask and the onebit registers
        uint32x4_t v_p1_nextbit_0 = vandq_s32(v_p1_mask_0, v_onebit);
        uint32x4_t v_p2_nextbit_0 = vandq_s32(v_p2_mask_0, v_onebit);
        uint32x4_t v_p1_nextbit_1 = vandq_s32(v_p1_mask_1, v_onebit);
        uint32x4_t v_p2_nextbit_1 = vandq_s32(v_p2_mask_1, v_onebit);

        // insert the bits using OR
        v_p1_0 = vorrq_s32(v_p1_0, v_p1_nextbit_0);
        v_p2_0 = vorrq_s32(v_p2_0, v_p2_nextbit_0);
        v_p1_1 = vorrq_s32(v_p1_1, v_p1_nextbit_1);
        v_p2_1 = vorrq_s32(v_p2_1, v_p2_nextbit_1);
    ...
    vst1q_s32(p2_lo, v_p2_0);
    vst1q_s32(p1_lo+4, v_p1_1);
    vst1q_s32(p2_lo+4, v_p2_1);
    ...
    if (C - (ic*cntbits) >= 32) {
        ...
        v_p2_0 = vdupq_n_s32(0);
        v_p2_1 = vdupq_n_s32(0);
        ...
        vst1q_s32(p2_hi, v_p2_0);
        vst1q_s32(p1_hi+4, v_p1_1);
        vst1q_s32(p2_hi+4, v_p2_1);
    } 
    // otherwise set the high 32-bits to zero
    else {
        for (int i=0; i<8; i++) {
...
static inline void t2r_ARMx4_packed(float* x, int64_t* qx_row, float* q_thresholds, int C, int H, int W, int in, int ic, int ih, int iw, int ih_pad, int iw_pad, int H_stride, int W_stride, int kH, int kW, int C_pack, int H_fused, int W_fused, int OH, int OW) {
...
static inline void t2r_ARMx4_1x1(float* x, int64_t* qx_row, float* q_thresholds, int C, int in, int ic, int C_pack) {
    ...
    int32_t p2_0 = vgetq_lane_s32(v_p2, 0);
    int32_t p2_1 = vgetq_lane_s32(v_p2, 1);
    int32_t p2_2 = vgetq_lane_s32(v_p2, 2);
    int32_t p2_3 = vgetq_lane_s32(v_p2, 3);
    ...
    // store to qx_row
    int idx = ((in * C_pack) + ((ic/2)/PACKING) * PACKING) * BITS + ((ic/2) % PACKING);
    int high = ic % 2;
    
    int32_t *data = (int32_t *)&qx_row[idx];
    data[high] = p1_0;
    data[high + PACKING*2] = p2_0;
...
static inline void t2r_basic_packed(float* x, int64_t* qx_row, float* q_thresholds, int64_t* onebit, int C, int H, int W, int in, int ic, int ih, int iw, int ih_pad, int iw_pad, int H_stride, int W_stride, int kH, int kW, int C_pack, int H_fused, int W_fused, int OH, int OW) {
...
int64_t *ternarize2row(float* x, float* q_thresholds, int N, int C, int H, int W, int H_pad, int W_pad, int kH, int kW, int H_stride, int W_stride) {
    ...
    if (H_pack == 1 && W_pack == 1 && C%128 == 0) {
        for (int in = 0; in < N; in++) {
            for (int ic = 0; ic < priChannel; ic++) {
                t2r_ARMx4_1x1(x, qx_row, q_thresholds, C, in, ic*2, C_pack);
                t2r_ARMx4_1x1(x, qx_row, q_thresholds, C, in, ic*2+1, C_pack);
            }
        }
        ...
        for (int in = 0; in < N; in++) {
            for (int ih = 0; ih < H; ih++) {
                ...
                for (int ic = 0; ic < priChannel; ic++) {
                    ...
                    for (; iw+3 < W; iw+=4) {
                        int iw_pad = iw + W_pad;
                        t2r_ARMx4_packed(x, qx_row, q_thresholds, C, H, W, in, ic, ih, iw, ih_pad, iw_pad, H_stride, W_stride, kH, kW, C_pack, H_fused, W_fused, OH, OW);
                    }
                    for (; iw < W; iw++) {
                        int iw_pad = iw + W_pad;
                        t2r_basic_packed(x, qx_row, q_thresholds, onebit, C, H, W, in, ic, ih, iw, ih_pad, iw_pad, H_stride, W_stride, kH, kW, C_pack, H_fused, W_fused, OH, OW);
                    }
                }
                ...
                if ((C % cntbits) > 0) {
                    ...
                    for (; iw+3 < W; iw+=4) {
                        int iw_pad = iw + W_pad;
                        t2r_ARMx4_packed(x, qx_row, q_thresholds, C, H, W, in, priChannel, ih, iw, ih_pad, iw_pad, H_stride, W_stride, kH, kW, C_pack, H_fused, W_fused, OH, OW);
                    }
                    for (; iw < W; iw++) {
                        int iw_pad = iw + W_pad;
                        t2r_basic_packed(x, qx_row, q_thresholds, onebit, C, H, W, in, priChannel, ih, iw, ih_pad, iw_pad, H_stride, W_stride, kH, kW, C_pack, H_fused, W_fused, OH, OW);
                    }
                }
            }
        }
...
int64_t *t2r_p2_opt(float* x, float* q_thresholds, int N, int C, int H, int W, int H_pad, int W_pad, int kH, int kW, int H_stride, int W_stride) {
    return ternarize2row(x, q_thresholds, N, C, H, W, H_pad, W_pad, kH, kW, H_stride, W_stride);
}

// Not implemented for ARM NEON
