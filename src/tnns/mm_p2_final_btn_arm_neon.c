...
static inline void btn_gemm_micro_arm_packedx2(int64_t* a, int64_t* b, int* y, int I1, int I2, int K, int N, int M1, int M2, int KB, float alpha, bool last) {
    int OHW = M1 / N;
    ...
    for (int j = 0; j < I2; j++) {
        ...
        for (int i = 0; i < I1; i++) {
            ...
            int cnt = 0;
            for (; k + 7 < K; k += 8) {
                int64x2_t a_upper = vld1q_s64((int64_t*)(a + (i*KB + k + 0)/BITS));
                int64x2_t a_upper_1 = vld1q_s64((int64_t*)(a + (i*KB + k + 4)/BITS));
...
float* btn_gemm_packing1(int64_t* a, int64_t* b, int N, int OH, int OW, int KN, int K, float prelu_alpha) {
    int* y = calloc(N * KN * OH * OW, sizeof(int));
    const int M1 = N * OH * OW;
    const int M2 = KN;
    const int KB = K * BITS;

    btn_gemm_micro_arm_packedx2(a, b, y, M1, M2, KB, N, M1, M2, KB, prelu_alpha, true);
...
float* mm_btn(int64_t* a, int64_t* b, int N, int OH, int OW, int KN, int K, float prelu_alpha) {
    K = 2 * ((K + 1) / 2);
    return btn_gemm_packing1(a, b, N, OH, OW, KN, K, prelu_alpha);
}
